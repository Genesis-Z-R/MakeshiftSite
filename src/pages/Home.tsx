...
const Home: React.FC = () => {
  ...
  const fetchListings = useCallback(async (isNewSearch = false) => {
    ...
    try {
      ...
      if (isNewSearch) announce(`Found ${newItems.length} listings`);
    } catch (error) {
  ...
  }, [search, category, sort, offset, announce]);
            ...
            <header className="mb-8 space-y-6">
              <form onSubmit={handleSearch} className="relative group">
                ...
                <input
                  ...
                  className="w-full h-20 pl-16 pr-8 rounded-[2.5rem] bg-white dark:bg-slate-900 border-none shadow-sm text-lg font-bold text-slate-900 dark:text-slate-50 placeholder:text-slate-400 focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/30 transition-all"
                />
              </form>
            </header>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                    {listings.map((listing) => (
                      <Link
                        ...
                        className="group bg-white dark:bg-slate-900 rounded-[2.5rem] p-4 shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-none transition-all duration-300"
                      >
                        ...
                        <div className="px-2 space-y-1">
                          ...
                          <h3 className="text-lg font-black text-slate-900 dark:text-slate-50 truncate group-hover:text-indigo-600 transition-colors">
                            {listing.title}
                          </h3>
                          <p className="text-sm font-bold text-slate-400 mb-4">by {listing.seller_name}</p>
